...
export default function WhatsAppCenter() {
  ...
  const patientTitle = term('patient', 'Patient');
  const campaignsLabel = term('campaigns', 'Campaigns');
  const industryTabs = useMemo(() => [
    ['connectivity', 'Connectivity', Wifi], ['inbox', `${patientTitle} Inbox`, MessageCircle], ['campaigns', campaignsLabel, Target],
  ...
  const launchCampaign = async id => {
    ...
    setCampaigns(await fetchApi('/whatsapp/campaigns')); setNotice(`${result.sentCount} ${patientLabel} messages queued.`);
  };
  ...
  const createJourney = async () => {
    ...
    setAutomations(await fetchApi('/whatsapp/automations')); setJourney({ ...journey, name: '', templateId: '' }); setNotice(`${patientTitle} journey created.`);
  };
  const runJourneyCycle = async () => {
    const result = await fetchApi('/whatsapp/automations/run', { method: 'POST', body: '{}' });
    setNotice(`${result.sentCount} automated ${patientLabel} messages queued.`);
  };
  ...
  const saveConsent = async marketingOptIn => {
    await fetchApi(`/whatsapp/contacts/${selected.id}/consent`, { method: 'PUT', body: JSON.stringify({ marketingOptIn }) }); setNotice(marketingOptIn ? 'Marketing consent recorded.' : `${patientTitle} opted out of marketing.`); setProfile(await fetchApi(`/whatsapp/contacts/${selected.id}`));
  };
    ...
    <div className="space-y-5 pb-10 text-slate-800 dark:text-slate-100">
      <section className="overflow-hidden rounded-3xl bg-[linear-gradient(120deg,#052e2b,#0f766e_60%,#115e59)] p-6 text-white shadow-xl">
        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
          <div>
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-emerald-200"><Sparkles className="h-4 w-4" /> {patientTitle} Engagement Suite</div>
            <h2 className="mt-3 text-2xl font-black tracking-tight md:text-3xl">WhatsApp Communication Center</h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-emerald-50/80">A calm daily workspace for {patientLabel} support, follow-ups, retention journeys and approved {campaignsLabel.toLowerCase()}.</p>
          </div>
      ...
      {!features.whatsappEnabled && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-800">
          WhatsApp is not enabled for this package yet. Ask the platform superadmin to enable connectivity, marketing and automation limits.
        </div>
      )}
      <div className="flex gap-2 overflow-x-auto pb-1">
        {industryTabs.map(([id, label, Icon]) => <button key={id} onClick={() => setTab(id)} className={`flex shrink-0 items-center gap-2 rounded-xl px-3.5 py-2.5 text-xs font-bold transition ${tab === id ? 'bg-slate-900 text-white shadow dark:bg-white dark:text-slate-900' : 'border border-slate-200 bg-white text-slate-500 dark:border-white/10 dark:bg-slate-900 dark:text-slate-300'}`}><Icon className="h-4 w-4" />{label}</button>)}
      </div>
...
function Connectivity({ features, settings, setSettings, saveSettings, health, diagnostics, testPhone, setTestPhone, testMessage, retryQueue, syncTemplates }) {
  const { term } = useClinic();
  const clinicLabel = term('clinic', 'business');
  const steps = [
...
function Inbox({ features, contacts, selected, setSelected, search, setSearch, profile, conversation, messages, composer, setComposer, sendMessage, suggestReply, suggesting, saveConversation, saveConsent }) {
  ...
  const quickMessages = {
    confirmation: `Assalam-o-Alaikum. Your ${appointmentLabel} is confirmed. Reply if you need any assistance.`,
    appointment: `Assalam-o-Alaikum. This is a friendly reminder about your upcoming ${appointmentLabel}. Please reply to confirm or request a reschedule.`,
    reminder_2h: `Assalam-o-Alaikum. Your ${appointmentLabel} is in 2 hours. We look forward to seeing you.`,
    billing: 'Assalam-o-Alaikum. This is a gentle payment reminder. Please let us know if you need an invoice or billing support.',
    clinical: `Assalam-o-Alaikum. We hope everything is going well after your ${treatmentLabel}. Please reply if you have any concerns or need guidance.`,
    review: 'Thank you for visiting us. Your feedback matters. May we send you our review link?',
    recommendation: `Assalam-o-Alaikum. Based on your previous ${visitLabel}, our team has a personalized ${treatmentLabel} recommendation for you. May we share the details?`,
  };
  ...
  return <div className="grid min-h-[680px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900 lg:grid-cols-[260px_1fr_285px]">
    <aside className="border-b border-slate-200 dark:border-white/10 lg:border-b-0 lg:border-r">
      <div className="p-4"><div className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-slate-400" /><Input value={search} onChange={e => setSearch(e.target.value)} placeholder={`Search ${patientLabel}s...`} style={{ paddingLeft: 36 }} /></div></div>
      <div className="max-h-[250px] overflow-auto lg:max-h-[610px]">{contacts.map(c => <button key={c.id} onClick={() => setSelected(c)} className={`w-full border-t border-slate-100 p-3 text-left transition dark:border-white/5 ${selected?.id === c.id ? 'bg-emerald-50 dark:bg-emerald-500/10' : 'hover:bg-slate-50 dark:hover:bg-white/5'}`}><div className="flex justify-between gap-2"><span className="text-sm font-bold">{c.name}</span><span className="text-[10px] text-slate-400">{c.appointmentCount} {visitLabel}s</span></div><p className="mt-1 truncate text-xs text-slate-500">{c.lastMessage || c.phone || 'No conversation yet'}</p></button>)}</div>
    </aside>
    <main className="flex min-h-[560px] flex-col">
      <div className="border-b border-slate-200 px-4 py-3 dark:border-white/10"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-black">{selected?.name || `Select a ${patientLabel}`}</p><p className="text-xs text-slate-500">{patientTitle} support and coordination</p></div><Pill tone={freeReply ? 'emerald' : 'amber'}>{freeReply ? 'Free reply window open' : 'Approved template required'}</Pill></div></div>
      <div className="flex-1 space-y-3 overflow-auto bg-slate-50/70 p-4 dark:bg-slate-950/40">{messages.length ? messages.map(m => <div key={m.id} className={`flex ${m.direction === 'outbound' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[80%] rounded-2xl px-3 py-2.5 text-xs leading-5 ${m.direction === 'outbound' ? 'rounded-br-sm bg-emerald-600 text-white' : 'rounded-bl-sm bg-white shadow dark:bg-slate-800'}`}><p>{m.body}</p><span className="mt-1 block text-[9px] opacity-70">{m.purpose} · {m.deliveryStatus}</span></div></div>) : <div className="grid h-full place-items-center text-center text-sm text-slate-400">Start a helpful, privacy-conscious {patientLabel} conversation.</div>}</div>
    ...
    <aside className="border-t border-slate-200 p-4 dark:border-white/10 lg:border-l lg:border-t-0"><p className="text-xs font-black uppercase tracking-widest text-slate-400">{patientTitle} Profile</p><h3 className="mt-3 text-lg font-black">{profile?.client?.name || `${patientTitle} context`}</h3><p className="text-xs text-slate-500">{profile?.client?.phone}</p><div className="mt-4 grid grid-cols-2 gap-2"><Mini label="Lifetime value" value={money(profile?.client?.totalSpent)} /><Mini label="Balance" value={money(profile?.client?.outstandingBalance)} /><Mini label="Loyalty" value={profile?.client?.loyaltyTier || '-'} /><Mini label={`${visitLabel}s`} value={profile?.appointments?.length || 0} /></div><ConversationControls conversation={conversation} profile={profile} saveConversation={saveConversation} saveConsent={saveConsent} /><p className="mt-5 text-xs font-black uppercase tracking-widest text-slate-400">Unified Timeline</p><div className="mt-2 space-y-2">{profile?.appointments?.slice(0, 4).map(a => <div key={a.id} className="rounded-xl bg-slate-50 p-3 text-xs dark:bg-white/5"><p className="font-bold">{a.serviceName || appointmentLabel}</p><p className="mt-1 text-slate-500">{a.date} · {a.staffName || doctorLabel}</p></div>)}</div></aside>
  </div>;
}
function ConversationControls({ conversation, profile, saveConversation, saveConsent }) { const { term } = useClinic(); const doctorLabel = term('doctor', 'Doctor'); const treatmentLabel = term('treatment', 'Treatment'); const [form,setForm]=useState({ assignedTo:'Reception',preferredLanguage:'en',internalNote:'' }); useEffect(()=>setForm({ assignedTo:conversation?.assignedTo||'Reception',preferredLanguage:conversation?.preferredLanguage||'en',internalNote:conversation?.internalNote||'' }),[conversation?.id,conversation?.assignedTo,conversation?.preferredLanguage,conversation?.internalNote]); return <div className="mt-4 space-y-2 border-t border-slate-100 pt-4 dark:border-white/10"><p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Conversation controls</p><Select value={form.assignedTo} onChange={e=>setForm({...form,assignedTo:e.target.value})}><option>Reception</option><option>{doctorLabel}</option><option>{treatmentLabel} Coordinator</option></Select><Select value={form.preferredLanguage} onChange={e=>setForm({...form,preferredLanguage:e.target.value})}><option value="en">English</option><option value="ur">Urdu</option></Select><textarea className="w-full rounded-xl border border-slate-200 p-2 text-xs dark:border-white/10 dark:bg-slate-950" rows="2" placeholder="Internal note" value={form.internalNote} onChange={e=>setForm({...form,internalNote:e.target.value})}/><Button onClick={()=>saveConversation(form)} className="w-full py-2">Save assignment</Button><button onClick={()=>saveConsent(!Number(profile?.client?.whatsappMarketingOptIn))} className={`w-full rounded-xl px-3 py-2 text-xs font-bold ${Number(profile?.client?.whatsappMarketingOptIn)?'bg-emerald-50 text-emerald-700':'bg-slate-100 text-slate-500'}`}>{Number(profile?.client?.whatsappMarketingOptIn)?'Marketing consent: opted in':'Marketing consent: opted out'}</button></div>; }
...
function Analytics({ metrics, campaigns }) { const { term } = useClinic(); const appointmentsLabel = term('appointments', 'Appointments'); const campaignLabel = term('campaigns', 'Campaigns'); const treatmentLabel = term('treatment', 'treatment'); const cards = [['Messages sent', metrics.sent, Send], ['Delivery rate', `${metrics.deliveryRate || 0}%`, CheckCircle2], ['Read rate', `${metrics.readRate || 0}%`, Activity], ['Response rate', `${metrics.responseRate || 0}%`, MessageCircle], [`${appointmentsLabel} generated`, metrics.conversions || 0, Target], ['Revenue generated', money(metrics.revenue), CircleDollarSign], ['API usage cost', money(metrics.cost), BarChart3], [campaignLabel, campaigns.length, Sparkles]]; return <div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{cards.map(([label, value, Icon]) => <Card key={label} className="p-4"><Icon className="h-5 w-5 text-emerald-600" /><p className="mt-5 text-2xl font-black">{value}</p><p className="mt-1 text-xs font-bold text-slate-500">{label}</p></Card>)}</div><Card className="mt-4 p-5"><h3 className="font-black">Retention intelligence</h3><p className="mt-2 text-sm leading-6 text-slate-500">Delivery, read, response, conversion, {treatmentLabel} booking, revenue and ROI events are structured for attribution. Live webhook status events populate delivery and read performance.</p></Card></div>; }
function ApiSettings({ features, settings, setSettings, saveSettings, branches, saveBranch }) { const { term } = useClinic(); const patientLabel = term('patient', 'patient'); const appointmentLabel = term('appointment', 'appointment'); const treatmentLabel = term('treatment', 'treatment'); const clinicLabel = term('clinic', 'business'); return <div className="grid gap-4 xl:grid-cols-[1fr_360px]"><Card className="p-5"><div className="flex items-center gap-2"><Settings2 className="h-5 w-5 text-emerald-600" /><h3 className="text-lg font-black">{clinicLabel} WhatsApp number settings</h3></div><p className="mt-2 text-xs leading-5 text-slate-500">Meta app credentials and webhook secrets are managed by the platform superadmin. Users only maintain the assigned phone number IDs and quiet hours.</p><div className="mt-5 grid gap-3 md:grid-cols-2"><Input placeholder="Phone Number ID" value={settings.phoneNumberId || ''} onChange={e => setSettings({ ...settings, phoneNumberId: e.target.value })} /><Input placeholder="WhatsApp Business Account ID" value={settings.businessAccountId || ''} onChange={e => setSettings({ ...settings, businessAccountId: e.target.value })} /><Input placeholder="Graph API version" value={settings.apiVersion || ''} onChange={e => setSettings({ ...settings, apiVersion: e.target.value })} /><label className="flex items-center gap-3 rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-bold dark:border-white/10"><input type="checkbox" checked={Boolean(Number(settings.simulationMode))} onChange={e => setSettings({ ...settings, simulationMode: e.target.checked })} /> Simulation mode</label><Input type="time" value={settings.quietHoursStart || '21:00'} onChange={e => setSettings({ ...settings, quietHoursStart: e.target.value })} /><Input type="time" value={settings.quietHoursEnd || '09:00'} onChange={e => setSettings({ ...settings, quietHoursEnd: e.target.value })} /></div><Button disabled={!features.whatsappEnabled} onClick={saveSettings} className="mt-4"><LockKeyhole className="h-4 w-4" /> Save number settings</Button><BranchRouting branches={branches} saveBranch={saveBranch} /></Card><Card className="p-5"><ShieldCheck className="h-7 w-7 text-emerald-600" /><h3 className="mt-4 font-black">Communication controls</h3><div className="mt-4 space-y-3 text-xs leading-5 text-slate-500"><p>Support, {appointmentLabel}, {treatmentLabel}, billing and marketing messages stay classified separately.</p><p>Marketing consent is tracked per {patientLabel}. Incoming STOP automatically opts the {patientLabel} out.</p><p>Quiet hours, secure document links and approved templates protect the daily workflow.</p></div></Card></div>; }

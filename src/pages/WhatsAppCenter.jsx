...
const tabs = [
  ...
  ['settings', 'API Settings', Settings2],
];
...
export default function WhatsAppCenter() {
  ...
  const saveSettings = async () => {
    ...
    setSettings(s => ({ ...s, accessToken: '', hasAccessToken: result.hasAccessToken })); setNotice('Cloud API settings saved securely.');
  };
  ...
  return (
    <div className="space-y-5 pb-10 text-slate-800 dark:text-slate-100">
      ...
      {tab === 'connectivity' && <Connectivity settings={settings} setSettings={setSettings} saveSettings={saveSettings} health={health} diagnostics={diagnostics} testPhone={testPhone} setTestPhone={setTestPhone} testMessage={testMessage} retryQueue={retryQueue} syncTemplates={syncTemplates} />}
      {tab === 'inbox' && <Inbox contacts={visibleContacts} selected={selected} setSelected={setSelected} search={search} setSearch={setSearch} profile={profile} conversation={conversation} messages={messages} composer={composer} setComposer={setComposer} sendMessage={sendMessage} saveConversation={saveConversation} saveConsent={saveConsent} />}
      {tab === 'campaigns' && <Campaigns campaigns={campaigns} campaign={campaign} setCampaign={setCampaign} segments={segments} templates={templates} createCampaign={createCampaign} launchCampaign={launchCampaign} />}
      {tab === 'journeys' && <Journeys automations={automations} toggleAutomation={toggleAutomation} templates={templates} journey={journey} setJourney={setJourney} createJourney={createJourney} runJourneyCycle={runJourneyCycle} />}
      {tab === 'segments' && <Segments segments={segments} />}
      {tab === 'templates' && <Templates templates={templates} template={template} setTemplate={setTemplate} createTemplate={createTemplate} syncTemplates={syncTemplates} />}
      {tab === 'media' && <MediaLibrary media={media} />}
      {tab === 'analytics' && <Analytics metrics={dashboard.metrics || {}} campaigns={campaigns} />}
      {tab === 'settings' && <ApiSettings settings={settings} setSettings={setSettings} saveSettings={saveSettings} branches={branches} saveBranch={saveBranch} />}
    </div>
  );
}

function Connectivity({ settings, setSettings, saveSettings, health, diagnostics, testPhone, setTestPhone, testMessage, retryQueue, syncTemplates }) {
  const steps = [
  ...
  return <div className="space-y-4">
    <div className="grid gap-4 xl:grid-cols-[1fr_340px]">
      <Card className="p-5"><p className="text-xs font-black uppercase tracking-widest text-emerald-600">Guided setup wizard</p><h3 className="mt-2 text-xl font-black">Connect Meta WhatsApp in five simple steps</h3><div className="mt-5 grid gap-2 md:grid-cols-5">{steps.map(([label, done], i) => <div key={label} className={`rounded-xl border p-3 ${done ? 'border-emerald-200 bg-emerald-50 dark:bg-emerald-500/10' : 'border-slate-200 dark:border-white/10'}`}><p className="text-[10px] font-black text-slate-400">STEP {i + 1}</p><p className="mt-2 text-xs font-bold">{label}</p><p className={`mt-2 text-[10px] font-bold ${done ? 'text-emerald-600' : 'text-slate-400'}`}>{done ? 'Complete' : 'Pending'}</p></div>)}</div><div className="mt-5 grid gap-3 md:grid-cols-2"><Input placeholder="Phone Number ID" value={settings.phoneNumberId || ''} onChange={e => setSettings({ ...settings, phoneNumberId: e.target.value })} /><Input placeholder="WhatsApp Business Account ID" value={settings.businessAccountId || ''} onChange={e => setSettings({ ...settings, businessAccountId: e.target.value })} /><Input placeholder={settings.hasAccessToken ? 'Token stored - enter only to replace' : 'Permanent access token'} type="password" value={settings.accessToken || ''} onChange={e => setSettings({ ...settings, accessToken: e.target.value })} /><Input placeholder="Webhook verify token" value={settings.webhookVerifyToken || ''} onChange={e => setSettings({ ...settings, webhookVerifyToken: e.target.value })} /></div><div className="mt-4 flex flex-wrap gap-2"><Button onClick={saveSettings}><LockKeyhole className="h-4 w-4" /> Save connection</Button><Button onClick={syncTemplates} className="bg-slate-900 hover:bg-slate-800"><RefreshCw className="h-4 w-4" /> Sync Meta templates</Button></div></Card>
    ...
    <div className="grid gap-4 xl:grid-cols-[1fr_1fr]"><Card className="p-5"><h3 className="font-black">One-click test message</h3><p className="mt-1 text-xs text-slate-500">Send an approved confirmation template to your own WhatsApp number.</p><div className="mt-4 flex gap-2"><Input placeholder="+92 300 0000000" value={testPhone} onChange={e => setTestPhone(e.target.value)} /><Button onClick={testMessage}><TestTube2 className="h-4 w-4" /> Send test</Button></div></Card><Card className="p-5"><div className="flex items-center justify-between"><h3 className="font-black">Webhook diagnostics</h3><Button onClick={retryQueue} className="bg-slate-900 hover:bg-slate-800"><RefreshCw className="h-4 w-4" /> Retry queue</Button></div><div className="mt-3 space-y-2 text-xs text-slate-500">{diagnostics.queue?.length ? diagnostics.queue.slice(0, 3).map(item => <p key={item.id} className="rounded-lg bg-amber-50 p-2 text-amber-700"><AlertTriangle className="mr-1 inline h-3.5 w-3.5" /> {item.lastError}</p>) : <p className="rounded-lg bg-emerald-50 p-2 text-emerald-700">No failed messages waiting for retry.</p>}</div></Card></div>
  </div>;
}

function Inbox({ contacts, selected, setSelected, search, setSearch, profile, conversation, messages, composer, setComposer, sendMessage, saveConversation, saveConsent }) {
...
function ApiSettings({ settings, setSettings, saveSettings, branches, saveBranch }) { return <div className="grid gap-4 xl:grid-cols-[1fr_360px]"><Card className="p-5"><div className="flex items-center gap-2"><Settings2 className="h-5 w-5 text-emerald-600" /><h3 className="text-lg font-black">Meta WhatsApp Business Cloud API</h3></div><p className="mt-2 text-xs leading-5 text-slate-500">Credentials stay encrypted on the PHP backend and are never returned to the browser.</p><div className="mt-5 grid gap-3 md:grid-cols-2"><Input placeholder="Phone Number ID" value={settings.phoneNumberId || ''} onChange={e => setSettings({ ...settings, phoneNumberId: e.target.value })} /><Input placeholder="WhatsApp Business Account ID" value={settings.businessAccountId || ''} onChange={e => setSettings({ ...settings, businessAccountId: e.target.value })} /><Input placeholder={settings.hasAccessToken ? 'Access token stored - enter only to replace' : 'Permanent access token'} type="password" value={settings.accessToken || ''} onChange={e => setSettings({ ...settings, accessToken: e.target.value })} /><Input placeholder="Webhook verify token" value={settings.webhookVerifyToken || ''} onChange={e => setSettings({ ...settings, webhookVerifyToken: e.target.value })} /><Input placeholder="Graph API version" value={settings.apiVersion || ''} onChange={e => setSettings({ ...settings, apiVersion: e.target.value })} /><label className="flex items-center gap-3 rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-bold dark:border-white/10"><input type="checkbox" checked={Boolean(Number(settings.simulationMode))} onChange={e => setSettings({ ...settings, simulationMode: e.target.checked })} /> Simulation mode</label><Input type="time" value={settings.quietHoursStart || '21:00'} onChange={e => setSettings({ ...settings, quietHoursStart: e.target.value })} /><Input type="time" value={settings.quietHoursEnd || '09:00'} onChange={e => setSettings({ ...settings, quietHoursEnd: e.target.value })} /></div><Button onClick={saveSettings} className="mt-4"><LockKeyhole className="h-4 w-4" /> Save secure settings</Button><BranchRouting branches={branches} saveBranch={saveBranch} /></Card><Card className="p-5"><ShieldCheck className="h-7 w-7 text-emerald-600" /><h3 className="mt-4 font-black">Healthcare communication controls</h3><div className="mt-4 space-y-3 text-xs leading-5 text-slate-500"><p>Support, appointment, clinical, billing and marketing messages stay classified separately.</p><p>Marketing consent is tracked per patient. Incoming STOP automatically opts the patient out.</p><p>Quiet hours, secure document links and approved templates protect the daily workflow.</p></div></Card></div>; }

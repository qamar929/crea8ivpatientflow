  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-teal-100 bg-teal-50 p-5 text-sm text-teal-900">
        ...
        <p className="mt-1 text-xs leading-5">Upload source → map fields → validate duplicates → preview → import into Patients, Appointments, Treatments, Notes, or Financial Records.</p>
      </div>
      ...
      <Modal isOpen={showForm} onClose={() => { setShowForm(false); setEditJob(null); }} title={editJob ? 'Edit Import Job' : 'New Import Job'} size="lg">
        <div className="space-y-4">
          ...
          <textarea className="premium-input w-full rounded-lg px-3 py-2 text-sm" rows={4} placeholder='Field mapping JSON e.g. {"name":"Patient Name","phone":"Mobile"}' value={JSON.stringify(form.fieldMapping || {}, null, 2)} onChange={e => { try { setForm({ ...form, fieldMapping: JSON.parse(e.target.value || '{}') }); } catch { /* keep editing */ } }} />
